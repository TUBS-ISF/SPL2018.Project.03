...
public class UnitBox extends HBox {
	...
	private void init() {
		...
		// Export
		if (!PluginLoader.load(GuiExporter.class).isEmpty()) {
		...
		// Import
		if (!PluginLoader.load(GuiImporter.class).isEmpty()) {
		...
		addAllChilds(nameField, Space.hBoxSpace(), exportButton, importButton, editButton, deleteButton);
	}
	...
	private void addAllChilds(Node... childs) {
		for(Node child : childs)
			addChild(child);
	}

	private void addExportFormatButtons(VBox exportPopupBox) {
		List<GuiExporter> exporters = PluginLoader.load(GuiExporter.class);
	...
	private void addImportFormatButtons(VBox importPopupBox) {
		List<GuiImporter> importers = PluginLoader.load(GuiImporter.class);
